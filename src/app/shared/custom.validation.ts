...
export class CustomValidators {
  ...
  static overallScoreValidator(weightedScore:number, unweightedScore:number, lowerBound:number, upperBound:number): ValidatorFn {
    return (control: AbstractControl): ErrorPayload<ErrorType.invalidOverallScore> | null => {
      if (weightedScore < lowerBound || unweightedScore < lowerBound || weightedScore > upperBound || unweightedScore > upperBound) {
        return { invalidOverallScore: { weightedScore, unweightedScore, lowerBound, upperBound } };
      } else {
        return null;
      }
    };
